...
- **Skins/themes** (`THEMES`, `applyTheme`): `THEMES` is a table of theme objects (`retro`, `neon`, `pastel`, `pixel`), each with its own `colors` palette, `gridColor`, `bg`, and a `drawBlock(context, x, y, colorIndex, size, alpha)` implementation so each skin can vary the actual paint effect (flat fill, glow via `shadowBlur`, rounded rects, sub-block pixel texture), not just the color values. The global `drawBlock` used by `draw()`/`drawNext()` just delegates to `currentTheme.drawBlock(...)`, and `drawGrid()` reads `currentTheme.gridColor`. `applyTheme(id)` validates `id` against `THEMES` (falling back to `'retro'`), sets `currentTheme`, persists the choice to `localStorage` under `tetris.skin` (wrapped in try/catch for `file://`/storage-disabled environments), sets `document.body.dataset.skin` so `style.css` can vary canvas background/border per skin, and repaints immediately if the game is already running. The saved skin (or `'retro'` if none) is applied once at script load, before `init()`.
- **Input**: a single `keydown` listener switches on `e.code` for movement/rotation/soft-drop/hard-drop, plus `KeyP` for pause (handled first, works even while paused). It bails out early when the event's target is a form control (`SELECT`/`INPUT`/`BUTTON`/`TEXTAREA`, e.g. `#skin-select` or the restart button) so game shortcuts don't fire while that control is focused.
